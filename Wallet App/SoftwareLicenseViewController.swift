...
class SoftwareLicenseViewController: UIViewController,UITabBarDelegate,UITextFieldDelegate,UITextViewDelegate,UIAlertViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if isEdit
        {
            ...
            self.txtWebsite!.text=self.dataDic.valueForKey("Website") as String
            self.txtLicenseKey!.text=self.dataDic.valueForKey("LicenseKey") as String
    ...
    {
        var editResult = api_Database.genericQueryforDatabase(DATABASENAME, query: NSString(format:"update SoftwareLicense set SoftwareName=%@,OwnerName=%@,CompanyNmae=%@,PurchaseDate='%@',Email='%@',LicenseKey='%@',Website='%@',SupportEmail='%@',Phone='%@',Price='%@',Notes=%@ where UID='%@'",api_Database.escape(txtSoftwareName!.text),api_Database.escape(txtOwnerName!.text),api_Database.escape(txtCompanyName!.text),txtPurchaseDate!.text,txtEmail!.text,txtLicenseKey!.text,txtWebsite!.text,txtSupportEmail!.text,txtPhone!.text,txtPrice!.text,api_Database.escape(txtViewNote!.text),self.dataDic.valueForKey("UID") as String))
        if editResult
        {
    ...
    func done() //Save Data Insert New Data
    {
        ...
        if txtSoftwareName!.text.lengthOfBytesUsingEncoding(NSUTF8StringEncoding) == 0 || txtSoftwareName!.text.stringByTrimmingCharactersInSet(SpaceCharSet).lengthOfBytesUsingEncoding(NSUTF8StringEncoding) == 0
        {
            self.alert("Error", text: "Please Enter Software Name")
            self.txtSoftwareName!.becomeFirstResponder()
            return
        }
        if txtLicenseKey!.text.lengthOfBytesUsingEncoding(NSUTF8StringEncoding) == 0 || txtLicenseKey!.text.stringByTrimmingCharactersInSet(SpaceCharSet).lengthOfBytesUsingEncoding(NSUTF8StringEncoding) == 0
        {
        ...
        var result=api_Database.genericQueryforDatabase(DATABASENAME, query:NSString(format:"insert into SoftwareLicense(UID,SoftwareName,OwnerName,CompanyNmae,PurchaseDate,Email,LicenseKey,Website,SupportEmail,Phone,Price,Notes) values('%@',%@,%@,%@,'%@','%@','%@','%@','%@','%@','%@',%@)",NSUUID.UUID().UUIDString,api_Database.escape(txtSoftwareName!.text),api_Database.escape(txtOwnerName!.text),api_Database.escape(txtCompanyName!.text),txtPurchaseDate!.text,txtEmail!.text,txtLicenseKey!.text,txtWebsite!.text,txtSupportEmail!.text,txtPhone!.text,txtPrice!.text,api_Database.escape(txtViewNote!.text)) as String)
