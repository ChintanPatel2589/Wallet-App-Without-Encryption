...
class PassportViewController: UIViewController,UITabBarDelegate,UITextFieldDelegate,UITextViewDelegate,UIAlertViewDelegate,UIPickerViewDelegate,UIPickerViewDataSource {
   ...
   // @IBOutlet var txtGender : UITextField
    ...
    override func viewDidLoad() {
        ...
        self.txtViewOther!.layer.cornerRadius = 5
    ...
    @IBAction func tapGestureForGender(sender:UITapGestureRecognizer)
    {
    ...
    {
        var editResult = api_Database.genericQueryforDatabase(DATABASENAME, query: NSString(format:"update IDPassport set Name=%@,Country='%@',Category='%@',ExpMonth='%@',ExpYear='%@',IDNumber='%@',State='%@',Address1=%@,Address2=%@,City='%@',ZIP='%@',Gender='%@',DOB='%@',Occupation=%@,Company=%@,Other=%@,Notes=%@ where UID='%@'",api_Database.escape(txtName!.text),txtCountry!.text,txtCategory!.text,txtExpirationMonth!.text,txtExpirationYear!.text,txtNumber!.text,txtState!.text,api_Database.escape(txtAddress1!.text),api_Database.escape(txtAddress2!.text),txtCity!.text,txtZIP!.text,self.gender,self.btnSelectDate!.titleLabel.text,api_Database.escape(txtOccupation!.text),api_Database.escape(txtCompany!.text),api_Database.escape(txtViewOther!.text),api_Database.escape(txtViewNote!.text),self.dataDic.valueForKey("UID") as String))
        if editResult
        {
    ...
    func done() //Save Data Insert New Data
    {
        ...
        if txtCategory!.text.lengthOfBytesUsingEncoding(NSUTF8StringEncoding) == 0 || txtCategory!.text.stringByTrimmingCharactersInSet(SpaceCharSet).lengthOfBytesUsingEncoding(NSUTF8StringEncoding) == 0
        {
            self.alert("Error", text: "Please Select Category")
        ...
        if txtCountry!.text.lengthOfBytesUsingEncoding(NSUTF8StringEncoding) == 0 || txtCountry!.text.stringByTrimmingCharactersInSet(SpaceCharSet).lengthOfBytesUsingEncoding(NSUTF8StringEncoding) == 0
        {
        ...
        var result=api_Database.genericQueryforDatabase(DATABASENAME, query:NSString(format:"insert into IDPassport(UID,Name,Country,Category,ExpMonth,ExpYear,IDNumber,State,Address1,Address2,City,ZIP,Gender,DOB,Occupation,Company,Other,Notes) values('%@',%@,'%@','%@','%@','%@','%@','%@',%@,%@,'%@','%@','%@','%@',%@,%@,%@,%@)",NSUUID.UUID().UUIDString,api_Database.escape(txtName!.text),txtCountry!.text,txtCategory!.text,txtExpirationMonth!.text,txtExpirationYear!.text,txtNumber!.text,txtState!.text,api_Database.escape(txtAddress1!.text),api_Database.escape(txtAddress2!.text),txtCity!.text,txtZIP!.text,self.gender,self.btnSelectDate!.titleLabel.text,api_Database.escape(txtOccupation!.text),api_Database.escape(txtCompany!.text),api_Database.escape(txtViewOther!.text),api_Database.escape(txtViewNote!.text)) as String)
    ...
    func textFieldDidBeginEditing(textField: UITextField!)
    {
        ...
        if textField.tag == 3
        {
            ...
            self.toolBarTitle!.title = "- Select Category -"
            self.view.bringSubviewToFront(self.viewPicker)
            self.ArrayPickerItem = ["--","ID","Passport"]
            getUpPickerView()
            self.tmpTypeText = self.txtCategory!.text
    ...
    @IBAction func tooBarPickerViewCancel()
    {
        ...
        {
            if self.txtCategory!.text.lengthOfBytesUsingEncoding(NSUTF8StringEncoding) == 0
            {
                self.txtCategory!.text = ""
            }
            else
            {
                self.txtCategory!.text = self.tmpTypeText
            }
        }
    ...
    @IBAction func toolBarPickerViewDone()
    {
        ...
        if self.pickerOfType == "CardType"
        {
            self.txtCategory!.text = self.selectedPickerItem
        }
        else if self.pickerOfType == "MonthPicker"
        {
