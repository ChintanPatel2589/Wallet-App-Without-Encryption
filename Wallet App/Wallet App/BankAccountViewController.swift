...
{
    var editResult = api_Database.genericQueryforDatabase(DATABASENAME, query: NSString(format:"update bankAccount set BankName=%@,Name=%@,Category='%@',Currency='%@',AccountNumber='%@',BankCode='%@',Swift='%@',IBAN='%@',Pin='%@',Website='%@',Phone='%@',Address1=%@,Address2=%@,BranchName=%@,UserID='%@',Notes=%@ where UID='%@'",api_Database.escape(txtBankName!.text),api_Database.escape(txtOwnerName!.text),txtCategory!.text,"TEMP",txtNumber!.text,txtBankCode!.text,txtSwift!.text,txtIBAN!.text,txtPin!.text,txtiBankingWebSite!.text,txtPhone!.text,api_Database.escape(txtAddress1!.text),api_Database.escape(txtAddress2!.text),api_Database.escape(txtBranchName!.text),txtUserID!.text,api_Database.escape(txtViewNote!.text),self.dataDic.valueForKey("UID") as String))
    if editResult
    {
...
func done() //Save Data Insert New Data
{
    ...
    {
        self.alert("Error", text: "Please Enter A\\C Number")
    ...
    var result=api_Database.genericQueryforDatabase(DATABASENAME, query:NSString(format:"insert into bankAccount(UID,BankName,Name,Category,Currency,AccountNumber,BankCode,Swift,IBAN,Pin,Website,Phone,Address1,Address2,BranchName,UserID,Notes) values('%@',%@,%@,'%@','%@','%@','%@','%@','%@','%@','%@','%@',%@,%@,%@,'%@',%@)",NSUUID.UUID().UUIDString,api_Database.escape(txtBankName!.text),api_Database.escape(txtOwnerName!.text),txtCategory!.text,"TEMP",txtNumber!.text,txtBankCode!.text,txtSwift!.text,txtIBAN!.text,txtPin!.text,txtiBankingWebSite!.text,txtPhone!.text,api_Database.escape(txtAddress1!.text),api_Database.escape(txtAddress2!.text),api_Database.escape(txtBranchName!.text),txtUserID!.text,api_Database.escape(txtViewNote!.text)) as String)
    ...
    if textField.tag == 3
    {
        ...
        UIView.animateWithDuration(0.3, delay: 0, options: .CurveLinear, animations:
            ...
            completion: {
                ...
                {
                    self.txtCategory!.text=self.ArrayPickerItem.objectAtIndex(0) as String
                }
...
func textView(textView: UITextView!, shouldChangeTextInRange range: NSRange, replacementText text: String!) -> Bool
{
    ...
    {
        self.selectedType = self.ArrayPickerItem.objectAtIndex(row) as String
    }
    ...
    @IBAction func toolBarDone()
    {
        ...
        UIView.animateWithDuration(0.3, delay: 0, options: .CurveLinear, animations:
            ...
            completion: {
                (finished: Bool) in
                self.txtCategory!.text = self.selectedType
                println("finished")
            });
    }
